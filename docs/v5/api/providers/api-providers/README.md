...
- **string**: In this case this will be assumed to be the `clientID` property of your application. 
- **object**: In this case you will be required one of the following combinations: 
- `clientID`: If you only specify this property this will have the same effect as passing it as a **string**. 
- `clientID` and `applicationSecretKey`: If you specify the `applicationSecretKey`, you also need to specify the `clientID` property. 
...
provider = new PocketGatewayProvider(null, clientID);
provider = new PocketGatewayProvider("homestead", clientID);
